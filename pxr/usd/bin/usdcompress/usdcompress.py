...
class UsdDracoEncoder(object):
  ...
  def encodeMesh(self, stage, mesh, fileName):
    ...
    success = UsdDraco._WriteDraco(mesh, fileName,
                                   ...
                                   preserve_holes)
    if not success:
      sys.exit(f'Could not encode mesh: {str(mesh.GetPrim().GetPrimPath())}')
    ...
    for primvar in UsdGeom.PrimvarsAPI(mesh.GetPrim()).GetPrimvars():
      if UsdDraco._PrimvarSupported(primvar):
        ...
        self.removePropertyOrExit(stage, mesh, name)
        self.removePropertyOrExit(stage, mesh, f'{name}:indices')
  ...
  def warnOrExit(self, prop, plural):
    ...
    message = (f'{level}: Results may be invalid. {opinion}' +
  ...
  def getFileName(self, mesh):
    ...
    if self.usedFileNames[fileName] > 1:
      fileName += f'_{str(self.usedFileNames[fileName])}'
    return fileName
...
def ParseOptions():
  ...
  parser.add_argument(
      ...
      choices=range(31),
      metavar='0-30',
      help='quantization bits for positions',
  )
  parser.add_argument(
      '-qt',
      type=int,
      ...
      choices=range(31),
      metavar='0-30',
      help='quantization bits for textures',
  )
  parser.add_argument(
      '-qn',
      type=int,
      ...
      choices=range(31),
      metavar='0-30',
      help='quantization bits for normals',
  )
  parser.add_argument(
      '-cl',
      type=int,
      ...
      choices=range(11),
      metavar='0-10',
      help='compression level, best=10',
  )
  ...
  if options.verbose:
    ...
    print(f'  quantization bits for positions : {str(options.qp)}')
    print(f'  quantization bits for textures  : {str(options.qt)}')
    print(f'  quantization bits for normals   : {str(options.qn)}')
    print(f'  compression level : {str(options.cl)}')
...
def main():
  ...
  encoder.encodeStage(stage, f'{options.output}.draco/')
